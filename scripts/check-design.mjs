...
const base=process.env.DESIGN_TEST_URL||'http://127.0.0.1:4180';
...
try {
 ...
 for(const [name,width,height]of [['desktop',1440,1000],['ipad',820,1180],['phone',390,844],['small-phone',320,740]]){
   ...
   for(const view of ['home','signup']){for(const theme of ['light','dark']){
     await page.goto(`${base}/scripts/design-check.html?page=${view}&theme=${theme}`);
     ...
     results.push({name,view,theme,...checks});
     await page.screenshot({path:`screenshots/design/${view}-${name}-${theme}.png`,fullPage:true});
     if(view==='signup'){
