...
- T4 / AC4: unit/runtime/UI, browser responsiveness, disposable PostgreSQL and production build checks; deploy database before compatible frontend; verify live assets. Verified: bf860b4 deployed successfully to Cloudflare; production bundle contains all three new controls.
...
- Release verified: Cloudflare deployment `1c9228c8-6882-416a-9fa9-e17a139e49c3` succeeded for bf860b4. Production `https://koc-jungle.pages.dev/` serves `/assets/index-UEAQgKa8.js`, containing the planner and unfinished-match controls. A subsequent database dry run reports up to date. Website release only; iOS archive was still running, not an App Store submission.
