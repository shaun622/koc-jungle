...
-- The signup-first winner adds a wait-listed row but leaves the confirmed
-- roster stable. Re-preview exactly that confirmed roster using the new
-- event/signup revision tokens before the second, Start-first race.
do $$
declare
  original jsonb := (select payload from public.americano_test_concurrency_payloads where name='start-race');
...
begin
  if current_database() not like 'koc_americano_test_%' then
    raise exception 'Refusing refreshed Start setup outside an isolated database';
  end if;
  ...
  if event_row.id is null or event_row.state->>'status'<>'setup' or not signup.is_open or signup.roster_locked_at is not null then
    raise exception 'Signup-first result was not left safely in setup/open state';
  end if;
  if (select count(*) from public.signup_registrations where signup_event_id=signup.id and status='confirmed')<>4
    ...
    raise exception 'Signup-first result did not retain four confirmed entrants and one waiter';
  end if;
  ...
  fingerprint:=public.americano_v2_schedule_fingerprint(next_state);
  if fingerprint is null then raise exception 'Could not compute refreshed Start fingerprint'; end if;
  ...
  insert into public.americano_test_concurrency_payloads(name,payload)
    values('start-race-ready',jsonb_build_object('eventId',event_row.id::text,'eventRevision',event_row.revision,
...
select 'Refreshed Start preview prepared after signup-first winner' as result;
