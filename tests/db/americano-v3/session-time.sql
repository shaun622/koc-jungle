...
begin
  ...
  if public.americano_v3_score_summary('{"kind":"rally","scoreA":10,"scoreB":8,"endedEarly":true}',jsonb_set(config,'{scoring,allowUnfinished}','false'),true) is null then raise exception 'Existing event could not record a stopped match'; end if;
  if public.americano_v3_score_summary('{"kind":"rally","scoreA":10,"scoreB":8,"endedEarly":true}',config #- '{scoring,allowUnfinished}',true) is null then raise exception 'Event without setup option could not record a stopped match'; end if;
  if public.americano_v3_score_summary('{"kind":"rally","scoreA":20,"scoreB":8,"endedEarly":true}',config,true) is not null then raise exception 'Above-target stopped score accepted'; end if;
