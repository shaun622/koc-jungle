...
export function AmericanoResultEditorV3({ event, match, readOnly, correcting, onSave }: {
  ...
  return <article className="amv3-match">
    ...
    {event.formatConfig.scoring.allowUnfinished && <label className="amv3-finish-played"><input type="checkbox" checked={draft.endedEarly === true} disabled={locked} onChange={(e) => { setDraft({ ...draft, endedEarly: e.target.checked }); setError(''); }} /><span>Finish with score played<small>Use when time is up. Actual scores only; level scores are a draw.</small></span></label>}
